...
int	game_load_spries(t_game *game)
{
	...
	mlx = game->mlx_ptr;
	return load_sprite(mlx, WALL_XPM,&game->wall ) &&
	load_sprite(mlx, FLOOR_XPM,&game->floor ) &&
	load_sprite(mlx, COINS_XPM,&game->coins ) &&
	load_sprite(mlx, PLAYER_FRONT_XPM,&game->player ) &&
	load_sprite(mlx, OPEN_EXIT_XPM,&game->exit_open ) &&
	load_sprite(mlx, EXIT_CLOSED_XPM,&game->exit_closed );
}
